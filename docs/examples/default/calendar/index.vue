    ...
    <div class="tvue-calendar__body">
      <el-calendar ref="calendar"
        ...
        <template #date-cell="{data}">
          <div class="item"
               :class="{'is-xiu':corXiu(data.date),'is-select':data.isSelected}">
            <small class="tip"
                   v-if="corXiu(data.date)">休</small>
            <b class="title">
              {{ data.day.split('-').slice(1)[1] }}
            </b>
            <small class="subtitle">{{corDate(data.date)}}</small>
          </div>
        </template>
      </el-calendar>
    </div>
...
const month_list = new Array(12).fill(null).map((_, index) => ({
  label: `${index + 1}月`,
  value: index
}));
...
function handleYear (val) {
  currentDate.value = new Date(val, month.value, currentDate.value.getDate());
  select.value = '';
}

function handleMonth (val) {
  currentDate.value = new Date(year.value, val, currentDate.value.getDate());
  select.value = '';
}

function handleSelect (val) {
  const item = list.find(ele => ele.value === val);
  if (item) {
    if (val.indexOf('-') === -1) {
      let { month, day } = lunar2solar(val);
      if (item.filter) {
        let obj = item.filter.find(ele => ele === year.value.toString());
        if (obj) day = day - 1;
      }
      let format = calendar.lunar2solar(item.old ? year.value - 1 : year.value, month, day);
      currentDate.value = new Date(year.value, format.cMonth - 1, format.cDay);
    } else {
      const array = val.split('-');
      currentDate.value = new Date(year.value, array[0] - 1, array[1]);
    }
  }
}

function corXiu (date) {
  ...
  list.forEach(ele => {
    ele.list.forEach(item => {
      if (item.indexOf('-') === -1) {
        ...
        if (mm === parseInt(array[0], 10) && dd === parseInt(array[1], 10)) {
          result = true;
        }
      }
    });
